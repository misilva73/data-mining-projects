...
array CLUS8vads [2] _temporary_;
drop _vqclus _vqmvar _vqnvar;
_vqmvar = 0;
do _vqclus = 1 to 2; CLUS8vads [_vqclus] = 0; end;
if not missing( T_newPremHealth ) then do;
   CLUS8vads [1] + ( T_newPremHealth - -0.50872279165006 )**2;
   CLUS8vads [2] + ( T_newPremHealth - 0.60847900418104 )**2;
end;
else _vqmvar + 1;
if not missing( T_newPremHousehold ) then do;
   CLUS8vads [1] + ( T_newPremHousehold - -0.51505369245157 )**2;
   CLUS8vads [2] + ( T_newPremHousehold - 0.61605134078266 )**2;
end;
else _vqmvar + 1;
if not missing( T_newPremLife ) then do;
   CLUS8vads [1] + ( T_newPremLife - -0.53587579857008 )**2;
   CLUS8vads [2] + ( T_newPremLife - 0.64095648480965 )**2;
end;
else _vqmvar + 1;
if not missing( T_newPremMotor ) then do;
   CLUS8vads [1] + ( T_newPremMotor - 0.7585724788734 )**2;
   CLUS8vads [2] + ( T_newPremMotor - -0.90732208998715 )**2;
end;
else _vqmvar + 1;
if not missing( T_newPremWork ) then do;
   CLUS8vads [1] + ( T_newPremWork - -0.53010825120418 )**2;
   CLUS8vads [2] + ( T_newPremWork - 0.63405797046085 )**2;
end;
else _vqmvar + 1;
_vqnvar = 5 - _vqmvar;
